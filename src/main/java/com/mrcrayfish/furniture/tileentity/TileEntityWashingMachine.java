...
implements ISidedInventory {
    ...
    public void startWashing() {
        if (this.canWash()) {
            if (this.timeRemaining == 0) {
                this.superMode = this.inventory[4].getItem() == MrCrayfishFurnitureMod.itemSuperSoapyWater;
                this.inventory[4] = new ItemStack(this.inventory[4].getItem().getContainerItem());
    ...
    public boolean canWash() {
        ...
        if (this.inventory[4] != null && this.timeRemaining == 0 && this.inventory[4].getItem() != MrCrayfishFurnitureMod.itemSoapyWater && this.inventory[4].getItem() != MrCrayfishFurnitureMod.itemSuperSoapyWater) {
            return false;
        }
        boolean flag = false;
        for (int i = 0; i < 4; ++i) {
    ...
    public void updateEntity() {
        if (this.washing) {
            ...
            if (this.timer == 0) {
                this.worldObj.playSoundEffect((double)this.xCoord, (double)this.yCoord, (double)this.zCoord, "cfm:washing_machine", 0.75f, 1.0f);
            }
            ++this.timer;
        }
    }
