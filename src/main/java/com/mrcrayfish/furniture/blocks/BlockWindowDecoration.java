...
extends Block {
    public BlockWindowDecoration(Material material) {
        super(material);
    ...
    public Item getItemDropped(int i, Random random, int j) {
        if (this == MrCrayfishFurnitureMod.blinds || this == MrCrayfishFurnitureMod.blindsClosed) {
            return MrCrayfishFurnitureMod.itemBlinds;
        }
        if (this == MrCrayfishFurnitureMod.curtains || this == MrCrayfishFurnitureMod.curtainsClosed) {
    ...
    public void onBlockPlacedBy(World par1World, int par2, int par3, int par4, EntityLivingBase par5EntityLivingBase, ItemStack par6ItemStack) {
        ((EntityPlayer)par5EntityLivingBase).triggerAchievement((StatBase)FurnitureAchievements.privacy);
    }

    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer, int par6, float par7, float par8, float par9) {
        int l = par1World.getBlockMetadata(par2, par3, par4);
        if (this == MrCrayfishFurnitureMod.blinds) {
            par1World.setBlock(par2, par3, par4, MrCrayfishFurnitureMod.blindsClosed, l, 3);
        }
        if (this == MrCrayfishFurnitureMod.curtains) {
            par1World.setBlock(par2, par3, par4, MrCrayfishFurnitureMod.curtainsClosed, l, 3);
        }
        return true;
    }
