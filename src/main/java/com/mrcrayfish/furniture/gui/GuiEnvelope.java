...
extends GuiContainer {
    ...
    public GuiEnvelope(InventoryPlayer inventoryplayer, IInventory inventoryMail, EntityPlayer player, ItemStack mail) {
        super((Container)new ContainerEnvelope((IInventory)inventoryplayer, inventoryMail));
        this.player = player;
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        this.fontRendererObj.drawString("Envelope", this.xSize / 2 - 22, 5, 9999999);
    ...
    public void initGui() {
        ...
        if (this.player.getCurrentEquippedItem() != null && (mail = this.player.getCurrentEquippedItem()).getItem() == MrCrayfishFurnitureMod.itemEnvelope) {
    ...
    protected void actionPerformed(GuiButton guibutton) {
        ...
        if (guibutton.id == 0 && this.player.getCurrentEquippedItem() != null) {
            NBTTagList list = (NBTTagList)NBTHelper.getCompoundTag(this.mail, "Envelope").getTag("Items");
            if (list.tagCount() > 0) {
                PacketHandler.INSTANCE.sendToServer((IMessage)new MessageEnvelope(this.mail));
