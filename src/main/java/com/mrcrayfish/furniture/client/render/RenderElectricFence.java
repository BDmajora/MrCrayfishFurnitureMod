...
public class RenderElectricFence
implements ISimpleBlockRenderingHandler {
    public void renderInventoryBlock(Block block, int metadata, int modelID, RenderBlocks renderer) {
    }

    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
        renderer.setOverrideBlockTexture(Blocks.anvil.getBlockTextureFromSide(2));
        renderer.setRenderBounds(0.4375, 0.0, 0.4375, 0.5625, (double)1.1f, 0.5625);
        renderer.renderStandardBlock(block, x, y, z);
        if (world.getBlock(x + 1, y, z) == MrCrayfishFurnitureMod.electricFence | world.getBlock(x + 1, y, z) == Blocks.fence_gate) {
        ...
        if (world.getBlock(x, y, z + 1) == MrCrayfishFurnitureMod.electricFence | world.getBlock(x, y, z + 1) == Blocks.fence_gate) {
